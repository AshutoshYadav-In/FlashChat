...
- Optional **self health ping**: server `GET` its own `/health` on a timer so the process receives HTTP traffic while awake (see `src/lib/keepAlive.js`). Enabled by default; set `KEEP_ALIVE_ENABLED=false` to turn off. Does not prevent a fully spun-down instance from sleeping; use an external uptime monitor for that.
...
| `KEEP_ALIVE_ENABLED` | Optional. Set to `false` to disable self-`/health` pings. Default: enabled. |
| `KEEP_ALIVE_INTERVAL_MS` | Optional. Default `120000` (2 minutes). |
